...
function Toasts({
  ...
  const grouped = useMemo(() => {
    const map = new Map<ToastPosition, typeof toasts>();
    map.set(defaultPosition, []);
    for (const toast of toasts) {
      const pos = getToastPosition(toast, defaultPosition);
      let arr = map.get(pos);
      if (!arr) {
        arr = [];
        map.set(pos, arr);
  ...
  return (
    <>
      {[...grouped.entries()].map(([position, groupToasts]) => {
        const swipeDirection = getSwipeDirection(position);
        return (
          <Toast.Portal key={position} data-slot="toast-portal" {...portalProps}>
            <Toast.Viewport
              className={cn(
...
const _rawToastManager = Toast.createToastManager();

type RawToastManager = typeof _rawToastManager;
type RawAddOptions = Parameters<RawToastManager["add"]>[0];

export type ToastAddOptions = RawAddOptions & {
  position?: ToastPosition;
};

export const toastManager: Omit<RawToastManager, "add"> & {
  add(options: ToastAddOptions): ReturnType<RawToastManager["add"]>;
} = Object.create(_rawToastManager, {
  add: {
    value(options: ToastAddOptions) {
      const { position, ...rest } = options;
      const existingData =
        typeof rest.data === "object" && rest.data !== null
          ? (rest.data as Record<string, unknown>)
          : {};
      return _rawToastManager.add({
        ...rest,
        data: { ...existingData, position },
      });
    },
    writable: true,
    configurable: true,
...
export function ToastProvider({
  ...
  return (
    <Toast.Provider toastManager={_rawToastManager} {...props}>
      {children}
